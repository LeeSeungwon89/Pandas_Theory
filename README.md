...
저는 파이썬을 배우고 다루는 일이 매우 어렵고 힘들었습니다. 쉬운 프로그래밍 언어로 유명한 파이썬이지만 비전공자에 처음 프로그래밍 언어를 접한 학습자 입장에서는 익히는 일이 여간 쉽지 않았습니다. 한 노력을 이어오다 보니 현재는 전문 개발자들처럼 마냥 능숙하지는 않지만 코드를 보고 해석할 수 있는 수준까지는 도달한 것으로 판단합니다.
